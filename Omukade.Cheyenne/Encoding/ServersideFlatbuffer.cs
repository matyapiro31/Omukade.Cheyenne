...
namespace Omukade.Cheyenne.Encoding
{
    public class ServersideFlatbufferEncoders
    {
        ...
        {
            DirectMatchInvitation? dmi = dmiModel == null ? null : new DirectMatchInvitation(
                dmiModel.Value.SourceAccountId,
                dmiModel.Value.MmToken,
                dmiModel.Value.IssuedAt,
                dmiModel.Value.ClientVersion,
                dmiModel.Value.TimeOffsetSeconds,
                dmiModel.Value.GetSharedContextBytes()?.ToArray(),
                dmiModel.Value.GetSignatureBytes()?.ToArray()
            );
            return dmi;
        }
    }
